...
async def start(client, message):
    ...
    if len(message.command) != 2:
        if PREMIUM_AND_REFERAL_MODE == True:
            buttons = [[
                ...
                InlineKeyboardButton('✇ Jᴏɪɴ Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ ✇', url=f'https://t.me/+q-PhLzazP-AzOGI1')
            ]]
        else:
            buttons = [[
                InlineKeyboardButton('✇ Jᴏɪɴ Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ ✇', url=CHNL_LNK),
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        for msg in msgs:
            ...
            try:
                ...
                if STREAM_MODE == True:
                    ...
                    button = [[
                        InlineKeyboardButton('👀 𝐰𝐚𝐭𝐜𝐡 𝐨𝐧𝐥𝐢𝐧𝐞 | 𝐃𝐨𝐰𝐧𝐥𝐨𝐚𝐝 𝐅𝐢𝐥𝐞 📥', url=f"https://t.me/Rockers_File_To_Stream_Bot")
                    ]]
        ...
        if is_valid == True:
            await message.reply_text(
                text=f"<strong>Hey {message.from_user.mention},\n\nYou are successfully verified\nNow you have unlimited access for all movies till today midnight. 24hours</strong>",
                protect_content=True
            )
    ...
    if not files_:
        ...
        try:
            ...
            title = '@ROCKERSBACKUP  ' + ' '.join(filter(lambda x: not x.startswith('www.') and not x.startswith('@'), file.file_name.split()))
...
async def shortlink(bot, message):
    ...
    try:
        ...
        return await message.reply_text("<b>Command Incomplete :(\n\nGive me a shortener website link and api along with the command !\n\nFormat: <code>/shortlink semple.in e3d82cdf8f9f4783c42170b515d1c271fb1c4500</code></b>")
...
async def check_plans_cmd(client, message):
    ...
    if await db.has_premium_access(user_id):         
        ...
        reply_markup = InlineKeyboardMarkup(btn)
        m=await message.reply_sticker("CAACAgUAAxkBAAJgDWZ1zZeQOxrJ9gmRCheqocKE3k1JAAIDAQACES7zMlxsgSNhILt2NQQ")         
