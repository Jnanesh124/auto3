        ...
        try:
            ...
            if not link:
                ...
                if PM_SEARCH == True:
                    ...
                    await message.reply_text(text=f"<b>ur requisted movie file and terabox link available in below group 👇</b>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📥 DOWNLOAD HERE 📥", url=f"https://t.me/+qvjQab9d8Yo4Yzc1")]]))
...
async def next_page(bot, query):
    ...
    if settings['button']:
        ...
        btn.insert(0, 
            [
                InlineKeyboardButton("🎞 𝐲𝐞𝐚𝐫", callback_data=f"years#{key}"),
                InlineKeyboardButton("📪 𝐄𝐩𝐢𝐬𝐨𝐝𝐞", callback_data=f"episodes#{key}"),
                InlineKeyboardButton("✉️ 𝐒𝐞𝐚𝐬𝐨𝐧𝐬",  callback_data=f"seasons#{key}")
            ]
        )
        btn.insert(0, [
...
async def auto_filter(client, name, msg, reply_msg, ai_search, spoll=False):
    ...
    if imdb:
        ...
        if settings["button"]:
            cap = f"<b>📪 𝐌𝐨𝐯𝐢𝐞 ━➣ {search}\n🪬 𝐏𝐞𝐫𝐬𝐨𝐧 ━➣ {message.from_user.mention}\n🏃 𝐒𝐩𝐞𝐞𝐝 ━➣ {remaining_seconds} sᴇᴄᴏɴᴅs\n\n</b>"
        else:
            cap = f"<b>📪 𝐌𝐨𝐯𝐢𝐞 ━➣ {search}\n🪬 𝐏𝐞𝐫𝐬𝐨𝐧 ━➣ {message.from_user.mention}\n🏃 𝐒𝐩𝐞𝐞𝐝 ━➣ {remaining_seconds} sᴇᴄᴏɴᴅs\n\n</b>"
